    ...
    public class RealWorldTerrainMapboxElevationGenerator : RealWorldTerrainElevationGenerator
    {
        ...
        private RealWorldTerrainMapboxElevationGenerator(int isx, int isy, int iex, int iey, int zoom)
        {
            ...
            for (int tx = isx; tx <= iex; tx++)
            {
                ...
                {
                    string filename = System.IO.Path.Combine(RealWorldTerrainEditorUtils.heightmapCacheFolder, zoom + "x" + tx + "x" + ty + ".pngraw");
        ...
        public static void Init()
        {
            ...
            float dpa = Mathf.Max(prefs.heightmapResolution * prefs.terrainCount.x, prefs.heightmapResolution * prefs.terrainCount.y) / Mathf.Max(range.x, range.y);
            int zoom = 0;
            ...
            RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, zoom, out stx, out sty);
            RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesTo.x, prefs.coordinatesTo.y, zoom, out etx, out ety);
            ...
            float dsx = range.x / (prefs.heightmapResolution - 1) * (Math.Abs(prefs.depthSharpness) > float.Epsilon ? prefs.depthSharpness : 1);
            depthStep = dsx * 100;
        }
