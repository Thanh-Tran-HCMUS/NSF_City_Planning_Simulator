...
namespace InfinityCode.RealWorldTerrain.Generators
{
    public static class RealWorldTerrainGrassGenerator
    {
        ...
        public static string url
        {
            get
            {
                string format = string.Format(RealWorldTerrainCultureInfo.numberFormat, "node({0},{1},{2},{3});way(bn)['landuse'~'grass|forest|meadow|park|pasture|recreation_ground'];(._;>;);out;node({0},{1},{2},{3});way(bn)['natural'~'scrub|wood']; (._;>;);out;node({0},{1},{2},{3});way(bn)['leisure'~'park|golf_course'];(._;>;);out;rel({0},{1},{2},{3})['leisure'~'golf_course'];(._;>;);out;>;out skel qt;rel({0},{1},{2},{3})['landuse'~'forest|park|farmland']; (._;>;);out;>;out skel qt;",
        ...
        public static string filename
        {
            get
            {
                return System.IO.Path.Combine(RealWorldTerrainEditorUtils.osmCacheFolder, string.Format("grass_{0}_{1}_{2}_{3}.osm", prefs.coordinatesTo.y, prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, prefs.coordinatesTo.x));
            }
        }
            ...
            if (!loaded)
            {
                ...
                foreach (KeyValuePair<string, RealWorldTerrainOSMWay> pair in ways)
                {
                    ...
                    if (w.HasTags("landuse", "grass", "farmland", "forest", "meadow", "park", "pasture", "recreation_ground") ||
                        w.HasTags("leisure", "park", "golf_course") || w.HasTags("natural", "scrub", "wood")) grassWays.Add(w);
                }
