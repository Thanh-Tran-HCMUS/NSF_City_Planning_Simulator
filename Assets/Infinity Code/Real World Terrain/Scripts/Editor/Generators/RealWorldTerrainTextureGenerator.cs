...
namespace InfinityCode.RealWorldTerrain.Generators
{
    public class RealWorldTerrainTextureGenerator
    {
        ...
        private RealWorldTerrainTextureGenerator(int tileX, int tileY, int zoom)
        {
            ...
            levelFolder = System.IO.Path.Combine(RealWorldTerrainEditorUtils.textureCacheFolder, zoom.ToString());
            if (!Directory.Exists(levelFolder)) Directory.CreateDirectory(levelFolder);

            filename = prefs.mapType.filePrefix + tileX + "x" + tileY + ".jpg";
            path = System.IO.Path.Combine(levelFolder, filename);
            string url;
        ...
        public static void GenerateTexture(RealWorldTerrainItem item)
        {
            if (string.IsNullOrEmpty(textureFilename))
            {
                ...
                textureFilename = System.IO.Path.Combine(RealWorldTerrainWindow.container.folder, item.name + "r" + textureWidth + "x" + textureHeight + "." + prefs.textureFileType);
            ...
            if (item.prefs.resultType == RealWorldTerrainResultType.terrain)
            {
                ...
                string path = System.IO.Path.Combine(RealWorldTerrainWindow.container.folder, item.name + "r" + textureWidth + "x" + textureHeight + ".terrainlayer");
        ...
        public static bool Init(int maxZoom)
        {
            ...
            {
                double bx, by, ex, ey;
                RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, zoom, out bx, out by);
                RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesTo.x, prefs.coordinatesTo.y, zoom, out ex, out ey);
