...
namespace InfinityCode.RealWorldTerrain.Generators
{
    public static class RealWorldTerrainBuildingGenerator
    {
        ...
        private static string url
        {
            get
            {
                string format = string.Format(RealWorldTerrainCultureInfo.numberFormat, "(way['building']({0},{1},{2},{3});relation['building']({0},{1},{2},{3}););out;>;out skel qt;", prefs.coordinatesTo.y, prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, prefs.coordinatesTo.x);
                string data = format;
        ...
        public static string filename
        {
            get
            {
                return System.IO.Path.Combine(RealWorldTerrainEditorUtils.osmCacheFolder, string.Format("buildings_{0}_{1}_{2}_{3}.osm", prefs.coordinatesTo.y, prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, prefs.coordinatesTo.x));
            }
        }
        ...
        private static void CreateHouse(RealWorldTerrainOSMWay way, RealWorldTerrainContainer globalContainer)
        {
            ...
            if (maxLng < prefs.coordinatesFrom.x ||
                maxLat < prefs.coordinatesTo.y || 
                minLng > prefs.coordinatesTo.x || 
                minLat > prefs.coordinatesFrom.y) return;
        ...
        private static void DownloadMultiRequests()
        {
            ...
            RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, multiRequestZoom, out tlx, out tly);
            RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesTo.x, prefs.coordinatesTo.y, multiRequestZoom, out brx, out bry);
            ...
            for (int x = itlx; x < ibrx; x++)
            {
                ...
                for (int y = itly; y < ibry; y++)
                {
                    ...
                    string fn = System.IO.Path.Combine(RealWorldTerrainEditorUtils.osmCacheFolder, string.Format("buildings_{0}_{1}_{2}_{3}.osm", bry, tlx, tly, brx));
                    string cfn = fn + "c";
        ...
        public static void GetGlobalPoints(List<Vector3> points, RealWorldTerrainContainer globalContainer)
        {
            ...
            {
                bool success;
                points[i] = RealWorldTerrainEditorUtils.CoordsToWorldWithElevation(points[i], globalContainer, Vector3.zero, out success);
                noValues[i] = !success;
