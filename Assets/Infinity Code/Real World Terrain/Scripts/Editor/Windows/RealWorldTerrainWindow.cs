...
namespace InfinityCode.RealWorldTerrain.Windows
{
    public class RealWorldTerrainWindow : EditorWindow
    {
        ...
        private static bool CheckFields(out Vector2 cStart, out Vector2 cEnd)
        {
            cStart = RealWorldTerrainUtils.LanLongToFlat(prefs.coordinatesFrom);
            cEnd = RealWorldTerrainUtils.LanLongToFlat(prefs.coordinatesTo);
            ...
            if (prefs.coordinatesFrom.x < -180 || prefs.coordinatesTo.x < -180 || prefs.coordinatesFrom.x > 180 || prefs.coordinatesTo.x > 180)
            {
                Debug.Log("Longitude must be between -180 and 180.");
                return false;
            }

            if (prefs.elevationProvider == RealWorldTerrainElevationProvider.SRTM && (cStart.y < 30 || cEnd.y > 150))
            {
        ...
        private static void PrepareStart()
        {
            ...
            Vector2 cStart;
            Vector2 cEnd;
            if (!CheckFields(out cStart, out cEnd)) return;

            Vector2 range = prefs.coordinatesTo - prefs.coordinatesFrom;
            ...
            {
                if (prefs.elevationProvider == RealWorldTerrainElevationProvider.SRTM) RealWorldTerrainSRTMElevationGenerator.Init(cStart, cEnd, range);
                ...
                if (prefs.generateTextures && prefs.resultType != RealWorldTerrainResultType.gaiaStamp)
                {
                    if (prefs.maxTextureLevel == 0)
                    {
                        ...
                        for (int z = 5; z < 24; z++)
                        {
                            ...
                            RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, z, out stx, out sty);
                            RealWorldTerrainUtils.LatLongToTile(prefs.coordinatesTo.x, prefs.coordinatesTo.y, z, out etx, out ety);
