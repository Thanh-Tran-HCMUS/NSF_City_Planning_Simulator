    ...
    public static class RealWorldTerrainEditorUtils
    {
        ...
        public static string assetPath
        {
            get
            {
                if (_assetPath == null)
                {
                    string[] dirs = Directory.GetDirectories("Assets", "Real World Terrain", SearchOption.AllDirectories);
            ...
            get
            {
                if (String.IsNullOrEmpty(_cacheFolder))
                {
                    ...
                    if (cache == "")
                    {
                        ...
                        if (parent != null) _cacheFolder = System.IO.Path.Combine(parent.ToString(), "RWT_Cache");
                    }
        ...
        public static string heightmapCacheFolder
        {
            get
            {
                if (String.IsNullOrEmpty(_heightmapCacheFolder))
                    _heightmapCacheFolder = System.IO.Path.Combine(cacheFolder, "Heightmaps");
        ...
        public static string historyCacheFolder
        {
            get
            {
                if (String.IsNullOrEmpty(_historyCacheFolder))
                    _historyCacheFolder = System.IO.Path.Combine(cacheFolder, "History");
        ...
        public static string osmCacheFolder
        {
            get
            {
                if (String.IsNullOrEmpty(_osmCacheFolder)) _osmCacheFolder = System.IO.Path.Combine(cacheFolder, "OSM");
        ...
        public static string textureCacheFolder
        {
            get
            {
                if (String.IsNullOrEmpty(_textureCacheFolder)) _textureCacheFolder = System.IO.Path.Combine(cacheFolder, "Textures");
        ...
        public static Vector3 CoordsToWorld(double mx, float y, double mz, RealWorldTerrainContainer globalContainer)
        {
            ...
            return new Vector3((float)mx, y * globalContainer.scale.y, (float)mz);
        }
        ...
        {
            double mx, my;
            RealWorldTerrainUtils.LatLongToMercat(point.x, point.z, out mx, out my);
            ...
            else if (globalContainer.prefs.elevationType == RealWorldTerrainElevationType.heightmap)
            {
                ...
                if (lastC2WItem == null || !lastC2WItem.Contains(point.x, point.z))
                {
                    lastC2WItem = null;
                    for (int i = 0; i < globalContainer.terrains.Length; i++)
                    {
                        if (globalContainer.terrains[i].Contains(point.x, point.z))
                        {
