...
#region General
        ...
        private static void Coordinates()
        {
            ...
            if (GUILayout.Button("Show Open Street Map"))
            {
                Vector2 center;
                int zoom;
                RealWorldTerrainUtils.GetCenterPointAndZoom(new []{prefs.coordinatesFrom, prefs.coordinatesTo}, out center, out zoom);
                Process.Start(string.Format("http://www.openstreetmap.org/#map={0}/{1}/{2}", zoom, center.y, center.x));
            }
        ...
        public static void InsertCoords()
        {
            ...
            try
            {
                ...
                {
                    prefs.coordinatesFrom.x = RealWorldTerrainXMLExt.GetAttribute<float>(fnode, "tlx");
                    prefs.coordinatesFrom.y = RealWorldTerrainXMLExt.GetAttribute<float>(fnode, "tly");
                    prefs.coordinatesTo.x = RealWorldTerrainXMLExt.GetAttribute<float>(fnode, "brx");
                    prefs.coordinatesTo.y = RealWorldTerrainXMLExt.GetAttribute<float>(fnode, "bry");
        ...
        private static void POI()
        {
            ...
            if (GUILayout.Button(new GUIContent("+", "Add POI"), EditorStyles.toolbarButton, GUILayout.ExpandWidth(false)))
            {
                prefs.POI.Add(new RealWorldTerrainPOI("New POI " + (prefs.POI.Count + 1),
                    (prefs.coordinatesFrom.x + prefs.coordinatesTo.x) / 2, (prefs.coordinatesFrom.y + prefs.coordinatesTo.y) / 2));
            }
